...
def multi_krum_patched(all_updates, n_attackers, multi_k=False):
    candidates = []
    candidate_indices = []
    ...
    if multi_k:
        while len(remaining_updates) > 2 * n_attackers + 2:
            ...
            remaining_updates = torch.cat(
                (remaining_updates[: indices[0]], remaining_updates[indices[0] + 1 :]),
                0,
            )
    else:
        torch.cuda.empty_cache()
...
def get_malicious_updates_fang(all_updates, model_re, n_attackers, multik):
    ...
    while lamda > threshold:
        ...
        if (multik and np.sum(krum_candidate < n_attackers) == n_attackers) or (
            (not multik) and krum_candidate < n_attackers
        ):  # step4，Oracle=True，恶意梯度都被选中(idx小于10的有10个)
            return mal_updates
        lamda *= 0.5
...
def adaptive_attack_mkrum(
    all_updates, model_re, n_attackers, multik, dev_type="unit_vec"
):
    # 固定扰动类型
    ...
    while torch.abs(lamda_succ - lamda) > threshold_diff:  # step3
        ...
        agg_grads, krum_candidate = multi_krum(
            mal_updates, n_attackers, multi_k=multik
        )  # agr,返回Krum聚合后的梯度，和被选中梯度的位置
        if (multik and np.sum(krum_candidate < n_attackers) == n_attackers) or (
            (not multik) and krum_candidate < n_attackers
        ):  # step4，Oracle=True，恶意梯度都被选中(idx小于10的有10个)
...
def tr_mean_patched(all_updates, beta):
    return torch.tensor(
        scipy.stats.trim_mean(all_updates.cpu().numpy(), beta, axis=0)
    ).cuda()
...
def tr_mean(all_updates, n_attackers):
    ...
    out = (
        torch.mean(sorted_updates[n_attackers:-n_attackers], 0)
        if n_attackers
        else torch.mean(sorted_updates, 0)
    )
    return out
...
def get_malicious_updates_fang_trmean(all_updates, model_re, n_attackers):
    ...
    )  # (2472266,2) wmax和恶意梯度上界
    min_range = torch.cat(((min_vector * min_)[:, None], min_vector[:, None]), dim=1)

    rand = (
        torch.from_numpy(np.random.uniform(0, 1, [len(deviation), n_attackers]))
        .type(torch.FloatTensor)
        .cuda()
    )  # 从(0,1)范围中随机采样 维度数*攻击者数 个随机数
...
def adaptive_attack_trmean(all_updates, model_re, n_attackers, dev_type="unit_vec"):
    if dev_type == "unit_vec":
    ...
    lamda = torch.Tensor(
        [10.0]
    ).cuda()  # compute_lambda_adaptive(all_updates, model_re, n_attackers)
...
def FLtrust(s_grad, all_grad):
    ...
    norm_grad = [
        all_grad[i] * torch.norm(s_grad) * relu_score[i] / torch.norm(all_grad[i])
        for i in range(all_grad.shape[0])
    ]
...
def foolsgold(grads):
    ...
    return torch.mm(
        torch.t(grads),
        torch.from_numpy(wv).type(torch.FloatTensor).unsqueeze(dim=1).cuda(),
    ).squeeze() / sum(wv)
...
def bulyan(all_updates, n_attackers):
    ...
    while len(bulyan_cluster) < (nusers - 2 * n_attackers):
        ...
        bulyan_cluster = (
            ...
            else torch.cat((bulyan_cluster, remaining_updates[indices[0]][None, :]), 0)
        )
        remaining_updates = torch.cat(
            (remaining_updates[: indices[0]], remaining_updates[indices[0] + 1 :]), 0
        )
    ...
    return torch.mean(sorted_params[: n - 2 * n_attackers], dim=0), np.array(
        candidate_indices
    )
