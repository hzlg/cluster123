...
#             print(
#                 "Classifier: ({batch}/{size}) Data: {data:.3f}s | Batch: {bt:.3f}s | | Loss: {loss:.4f} | top1: {top1: .4f} | top5: {top5: .4f}".format(
#                     batch=ind + 1,
#                     size=len_t,
#                     data=data_time.avg,
#                     bt=batch_time.avg,
...
def test(test_data, labels, model, criterion, use_cuda, debug_="MEDIUM", batch_size=64):
    ...
    with torch.no_grad():
        for ind in range(len_t):
            ...
            if debug_ == "HIGH" and ind % 100 == 0:
                print(
                    "Test classifier: ({batch}/{size}) Data: {data:.3f}s | Batch: {bt:.3f}s | Loss: {loss:.4f} | top1: {top1: .4f} | top5: {top5: .4f}".format(
                        batch=ind + 1,
                        size=len(test_data),
                        data=data_time.avg,
                        bt=batch_time.avg,
...
def test_patched(test_data, labels, model, criterion, use_cuda):
    model.eval()
    with torch.no_grad():
        if use_cuda:
            inputs, targets = test_data.cuda(), labels.cuda()
        inputs, targets = torch.autograd.Variable(inputs), torch.autograd.Variable(
            targets
        )
        outputs = model(inputs)
